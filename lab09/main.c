...
void tmr1_callback(OS_TMR *ptmr,void *p_arg) 
{
	...
	if(tcnt==5)
	{
 		LCD_ShowxNum(182,110,cpuusage/5,3,16,0);			//显示CPU使用率  
		cpuusage=0;
		tcnt=0; 
	}
	cpuusage+=OSCPUUsage;
	tcnt++;				    
 	LCD_ShowxNum(182,130,mem_perused(),3,16,0);	//显示内存使用率	 	  		 					    
	LCD_ShowxNum(182,150,((OS_Q*)(q_msg->OSEventPtr))->OSQEntries,3,16,0X80);//显示队列当前的大小		   
 }
...
{	
	...
	{
		case 0:
			LCD_Fill(0,0,119,99,RED);
			break;
		case 1:
			LCD_Fill(0,0,119,99,GREEN);
			break;
		case 2:		    
			LCD_Fill(0,0,119,99,BLUE);
			break;
		case 3:
			LCD_Fill(0,0,119,99,MAGENTA);
			break;
 		case 4:
			LCD_Fill(0,0,119,99,GBLUE);
			break;
		case 5:
			LCD_Fill(0,0,119,99,YELLOW);
			break;
		case 6:
			LCD_Fill(0,0,119,99,BRRED);
			break;	 
	}
	sta++;
	if(sta>6)sta=0;	 											   
...
void ucos_load_main_ui(void)
{
	...
	LCD_ShowString(10,110,200,16,16,"Mini STM32");	
	LCD_ShowString(10,130,200,16,16,"UCOSII TEST3");	
	LCD_ShowString(10,150,200,16,16,"ATOM@ALIENTEK");
  LCD_ShowString(10,175,240,16,16,"KEY_UP:ADJUST");	
  LCD_ShowString(10,195,240,16,16,"KEY0:Q ON/OFF   KEY1:TMR2 SW");	
 	LCD_DrawLine(0,100,lcddev.width,100);
	LCD_DrawLine(120,0,120,170);

 	LCD_DrawLine(0,170,lcddev.width,170);
 	LCD_DrawLine(0,220,lcddev.width,220);
	LCD_DrawLine(120,220,120,lcddev.height);
		    
 	LCD_ShowString(5,225,240,16,16,"QUEUE MSG");//队列消息
	LCD_ShowString(5,250,240,16,16,"Message:");	 
	LCD_ShowString(5+120,225,240,16,16,"FLAGS");//信号量集
	LCD_ShowString(5,125,240,16,16,"TOUCH");	//触摸屏
	LCD_ShowString(5,5,240,16,16,"TMR2");	//队列消息
	POINT_COLOR=BLUE;//设置字体为蓝色 
  	LCD_ShowString(150,110,200,16,16,"CPU:   %");	
   	LCD_ShowString(150,130,200,16,16,"MEM:   %");	
   	LCD_ShowString(150,150,200,16,16," Q :000");	
...
{	  	
	while(1)
	{
		...
		{	
		 	if(tp_dev.x[0]>120&&tp_dev.x[0]<lcddev.width&&tp_dev.y[0]<100)
			{													   
...
void qmsgshow_task(void *pdata)
{
	...
	while(1)
	{
		...
		LCD_ShowString(105,170,240,16,16,p);//显示消息
 		myfree(p);	  
		delay_ms(500);	 
	}									 
}
...
{							 
	...
	tmr4=OSTmrCreate(10,20,OS_TMR_OPT_PERIODIC,(OS_TMR_CALLBACK)tmr4_callback,0,"tmr4",&err);		//200ms执行一次
 	...
 	while(1)
	{
		...
		{
			flagsclrt--;
			if(flagsclrt==1)LCD_Fill(20,42,119,42+16,WHITE);//清除显示
		}
		switch(key)
		{ 
			...
			case KEY1_PRES://软件定时器2开关&触摸区域清空
				...
				{		    		    
 					...
 					LCD_ShowString(28,42,240,16,16,"TMR2 STOP");//提示定时器2关闭了	
				}
				LCD_Fill(120,1,lcddev.width-1,99,WHITE);//触摸区域清空
				break;	
			case WKUP_PRES://校准
...
{	
	...
	while(1)
	{
		...
		if(flags&0X0001)LCD_ShowString(140,262,240,16,16,"KEY0 DOWN  "); 
		if(flags&0X0002)LCD_ShowString(140,262,240,16,16,"KEY1 DOWN  "); 
		if(flags&0X0004)LCD_ShowString(140,262,240,16,16,"KEY_UP DOWN"); 
		LED1=0;
		delay_ms(50);
		LED1=1;
